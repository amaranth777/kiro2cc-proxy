...
- [★ 为什么本项目能支持 Claude 4.6+，而其他开源反代大多不能](#为什么本项目能支持-claude-46而其他开源反代大多不能)
- [项目概述与技术栈](#项目概述与技术栈)
- [目录结构](#目录结构)
...
## ★ 为什么本项目能支持 Claude 4.6+，而其他开源反代大多不能

> 承接上一节的模型路由：`map_model()` 只解决了"名字怎么转换"，但 Claude 4.6+ 真正跑起来还依赖下面六层能力。大多数开源反代项目只做了字符串替换和 SSE 转发这两件事，遇到 Kiro 这类私有二进制协议 + 新协议字段就直接 400/乱码，这也是"同样是反代，为什么别的项目不支持"的根本原因。

### 对比总览

| 能力层 | 典型开源反代（仅做字符串/SSE 转发） | 本项目 |
|---|---|---|
| 上游协议 | 假设上游是标准 SSE（`data: {...}\n\n`） | Kiro 返回 AWS Event Stream **私有二进制帧**，未解码直接是乱码 |
| 工具 Schema | 原样转发 Claude Code 传来的 JSON Schema | 展开 `$ref`、剔除 `anyOf/oneOf/allOf`，否则 Kiro 400 |
| Thinking 内容 | 假设有独立的 `thinking` SSE 事件类型 | Kiro 无此事件类型，思考内容混在纯文本流里，靠文本标签识别 |
| Thinking 协议字段 | 只认旧版 `budget_tokens` | 4.6+ 引入 `adaptive` + `effort`，需要双向转译 |
| 账号策略 | 单账号或简单轮询 | 多账号 × 4 端点桶正交轮询 + 会话粘滞路由 |
| 用量计费 | 直接透传或本地估算 | Kiro 无 cache 明细，四层降级链还原真实 usage |

### 层 1：AWS Event Stream 二进制帧解析——先解决"能不能读懂上游"

详细的帧结构、状态机见【难点突破 → 难点1】，此处只说明它与 4.6+ 支持的关系：大多数反代基于"上游是 OpenAI/Anthropic 兼容 SSE"的假设编写，收到 Kiro 返回的二进制帧时会尝试按 UTF-8 文本解析，直接得到乱码或程序崩溃——这与模型版本无关，是**协议层就走不通**，4.6/4.7/4.8 与更早版本一样过不去。

### 层 2：JSON Schema `$ref` 展开——4.6+ 工具定义更复杂后才暴露的坑

`src/anthropic/converter.rs:56` 的 `resolve_schema_refs()`：深度优先展开 `$ref`，支持 `#/$defs/` 与 `#/definitions/` 两种前缀，`MAX_REF_DEPTH = 16` 防止循环引用死循环。

Claude 4.6+ 客户端（尤其 Claude Code）生成的工具 Schema 更倾向于用 `$defs` 复用子结构，早期模型的工具定义大多是扁平结构。只做字符串替换的反代不展开 `$ref`，Kiro 收到未解析的 `$ref` 会直接 400，看起来像"新模型不支持工具调用"，实际是 Schema 没展开。

### 层 3：thinking 标签文本级重建——4.6+ 默认更积极使用 Extended Thinking 之后才会踩到

Kiro 没有独立的 `thinking` 事件类型，思考内容以 `<thinking>...</thinking>` 纯文本标签混在同一文本流里返回。`src/anthropic/stream.rs:69` 的 `find_real_thinking_end_tag()`（详见【难点突破 → 难点2】）通过"标签前后无引号 + 标签后跟 `\n\n`"两个条件排除被引用的假标签。旧模型 thinking 默认关闭，很少暴露这个问题；4.6+ 默认更积极地使用 extended thinking，不做这层重建会导致思考内容原样泄漏进最终回复。

### 层 4：adaptive thinking / effort 双向转译——4.6+ 独有的协议字段

这是 4.6+ 系列真正的"新协议"，也是最容易被忽视的一层。

**Anthropic 请求结构**（`src/anthropic/types.rs:71-117`）：

```rust
pub struct Thinking {
    pub thinking_type: String,   // "enabled" / "adaptive" / "disabled"
    #[serde(default = "default_budget_tokens", deserialize_with = "deserialize_budget_tokens")]
    pub budget_tokens: i32,       // 旧协议字段，4.6+ 用 adaptive 时不再需要
}
pub struct OutputConfig {
    #[serde(default = "default_effort")]
    pub effort: String,           // 4.6+ 新增：low/medium/high，替代 budget_tokens
}
```

**转译为 Kiro 认的格式**，两条独立通路（外加一层模型名自动兜底）缺一不可：

1. `build_additional_model_request_fields()`（`src/anthropic/converter.rs:1408`）：`thinking_type` 为 `"enabled"` 或 `"adaptive"` 都统一映射为 Kiro 认的协议字段 `"adaptive"`，`effort` 原样传入 `output_config`
2. `generate_thinking_prefix()`（`src/anthropic/converter.rs:1456`）：额外在对话文本前注入 `<thinking_mode>adaptive</thinking_mode><thinking_effort>{effort}</thinking_effort>`——因为 Kiro 后端实际是靠**文本提示词**驱动思考行为，协议字段本身更多是"声明"，两者缺一都会导致 4.6+ 的自适应思考不生效
3. `override_thinking_from_model_name()`（`src/anthropic/handlers.rs:1464`）：模型名含 `thinking` 时自动补全上述配置，用户无需手动传 `thinking` 字段

只实现旧版 `budget_tokens` 透传的反代，遇到 4.6+ 传来的 `effort` 字段会原样丢弃或报错，思考行为退化成"要么不思考，要么用一个固定 budget"，无法呈现 4.6+ 真正的自适应思考。

### 层 5：多账号 × 多端点故障转移——Opus 4.6+ 订阅门槛更高之后才关键

`src/kiro/provider.rs:643` 的 `select_endpoint()` 在 4 个限流桶间正交轮询（`src/kiro/endpoint.rs:23`）：

```rust
pub enum EndpointName {
    Ide,           // q.{region}.amazonaws.com，无 x-amz-target
    Runtime,       // runtime.{region}.kiro.dev，独立域
    Codewhisperer, // us-east-1 独占主机 / 其他区域走 q.* + x-amz-target
    Amazonq,       // q.{region}.amazonaws.com + x-amz-target
}
```

`call_api_with_retry()`（`src/kiro/provider.rs:673`）在此基础上叠加账号级重试（`max_retries = min(可用账号数×3, 9)`）。Opus 4.6+ 相比更早模型对账号订阅等级要求更高（`token_manager.rs` 中对 `credentials.supports_opus()` 的调用检查，函数定义见 `src/kiro/model/credentials.rs:376`），单账号/无故障转移的反代一旦这个账号没开通 Opus 4.6+ 权限就直接 403，而不会自动换账号重试。

### 层 6：Prompt Cache 用量四层降级——大窗口模型场景下才会失真

Kiro 的 `meteringEvent` 从不包含 cache 明细，`src/cache/mod.rs:33` 的 `select_final_usage()` 靠"计费真值 → 前缀估算 → 账号级指纹匹配 → 比例模拟"四层依次降级还原 usage。这一层的完整机制在《Kiro缓存与状态管理解析.md》中已有专门文档，此处不重复展开；它与 4.6+ 的关系是：1M 上下文窗口模型的用量基数更大，估算误差被放大，没有这层降级链的反代在大窗口场景下 `input_tokens` 汇报偏差会非常明显。

---

**结论**：Claude 4.6+ 支持不是靠"改一下模型名映射表"就能做到的——它同时要求协议层（二进制帧）、Schema 层（`$ref`）、内容流层（thinking 文本重建）、新字段层（adaptive/effort）、可用性层（多账号故障转移）、计费层（cache 降级）六层同时到位。大多数开源反代只做了其中"字符串转发 + SSE 转发"这一层，天然只能支持协议更简单的旧模型。
...
```rust
// file: src/anthropic/stream.rs:69 — find_real_thinking_end_tag()
